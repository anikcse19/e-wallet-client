...
import { getBuyerTradeOfferLists, markPaid } from "../../services/joinOfferList";
...
// console.log("hello")
    const fetchTradeLists = async () => {
      ...
      try {
        const data = await getBuyerTradeOfferLists(token);
        setTradeLists(data || []);
      } catch (error) {
...
console.log("tradelist",tradeLists)
  if (loading) return <div className="p-4">Loading...</div>;
  const handlePaid = async (tradeId) => {
    const token = localStorage.getItem("token"); // Or use context/state
    if (!token) return;
    console.log("token", token)
    try {
      const data = await markPaid(token, tradeId);
      if (data) {
